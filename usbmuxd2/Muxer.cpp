...
bool Muxer::hasDeviceManager() noexcept {
    return _wifidevmgr != NULL || _usbdevmgr != NULL || _manualdevmgr != NULL;
}
...
void Muxer::send_deviceList(std::shared_ptr<Client> cli, uint32_t tag) {
    ...
    plist_dict_set_item(p_rsp, "DeviceList", p_devarr);
    p_devarr = NULL;  // transfer ownership
...
void Muxer::send_listenerList(std::shared_ptr<Client> cli, uint32_t tag) {
    ...
    plist_dict_set_item(p_rsp, "ListenerList", p_cliarr);
    p_cliarr = NULL;  // transfer ownership
...
#warning TODO missing fields: InterfaceIndex (integer)
    ...
    plist_dict_set_item(p_devp, "Properties", p_props);
    p_props = NULL;  // transfer ownership
